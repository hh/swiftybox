...
/// Print the last NUM lines of each FILE to standard output
        ...
        while i < args.count {
            ...
            if arg == "-n" {
                ...
                guard i + 1 < args.count else {
                    FileHandle.standardError.write("tail: option requires an argument -- 'n'\n".data(using: .utf8)!)
                    return 1
                }
                i += 1
                guard let count = Int(args[i]) else {
                    FileHandle.standardError.write("tail: invalid number of lines: '\(args[i])'\n".data(using: .utf8)!)
                    return 1
                }
                lineCount = count
                ...
                if let count = Int(numStr) {
                    ...
                    FileHandle.standardError.write("tail: invalid option: '\(arg)'\n".data(using: .utf8)!)
                    return 1
                }
            } else {
                // It's a file
    ...
    private static func processFile(_ file: String, lineCount: Int) -> Int32 {
        ...
        if file == "-" {
            ...
            guard let data = try? FileHandle.standardInput.readToEnd(),
                ...
                FileHandle.standardError.write("tail: error reading standard input\n".data(using: .utf8)!)
                return 1
            }
            content = str
        } else {
            ...
            do {
                ...
                FileHandle.standardError.write("tail: cannot open '\(file)' for reading: No such file or directory\n".data(using: .utf8)!)
                return 1
            }
        }
        ...
        // Handle trailing newline - if content ends with \n, components will have empty string at end
        var actualLines = lines
        if content.hasSuffix("\n") && !lines.isEmpty && lines.last == "" {
            actualLines = Array(lines.dropLast())
        }

        let outputLines = actualLines.suffix(lineCount)
